...
BRed='\033[1;31m'   # BoldRed
wipe="\033[1m\033[0m" # wipe color
...
if [ "$WL_APP_NAME" == "whitelabel" ]; then
	printf "${BRed}ERROR: Provided invalid app name 'whitelabel'${wipe}"
	exit 0
fi
...
echo "> Copying 'whitelabel' directory to '${WL_APP_NAME}'"
...
printf "> Set app display name to ${BGreen}'${WL_DISPLAY_NAME}'${wipe} in directory ${BGreen}'${WL_DIR_NAME}'${wipe}\n"
...
printf "> Set bundle id to ${BGreen}'${WL_BUNDLE_ID}'${wipe}\n"
...
# pushd "./${WL_DIR_NAME}" > /dev/null
npm install --silent > /dev/null 2> /dev/null

printf "${BGreen}Success!${wipe}\n"
popd > /dev/null
